...
class ModelExtensionModuleOfd extends Model
{
    private function emailBugReport($message)
    {
        if($this->ofdGetOption('module_ofd_email')) {
            mail($this->ofdGetOption('module_ofd_email'),$this->language->get('text_error_ofd_mail'),$message);
        }
    }   
    ...
    public function echoError($message)
    {
        ...
        if($this->detectApi()||!$this->ofdIsAdmin()) {    
    ...
    public function setAuthToken()
    {
        ...
        if(isset($result->Status)&&($result->Status=='Success')) {
            $this->editSettingValue('module_ofd', 'module_ofd_token',$result->Data->AuthToken);
            $this->editSettingValue('module_ofd', 'module_ofd_token_exp_date', strtotime($result->Data->ExpirationDateUtc));
    ...
    {
        $data = array();
        $order_items = $this->model_checkout_order->getOrderProducts($order['order_id']);
        ...
        $totals = $this->model_checkout_order->getOrderTotals($order['order_id']);
        ...
        if($order) {
            if($this->checkExists($order_id,$type)) {    
                ...
                if($check_id = $this->sendDataToOFD($data)) {
                    ...
                    $this->language->set('text_success',sprintf($this->language->get('text_check_ok'),$temptype,$order_id));
                    return true;
                } else {
                    return false;
                }
